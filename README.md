...
开启“发送前识别并确认”后，同一个“发送到 MoviePilot”菜单会自动打开弹窗展示 MoviePilot 识别结果；核对后点击“发送到 MoviePilot”才会创建任务。标题或链接被修改后，必须重新识别。

“发送前识别并确认”开关会即时保存并重建右键菜单，无需再点击页面底部的“保存设置”。两种模式显示相同名称，但使用互斥的菜单 ID；直发入口不会调用扩展弹窗。
